...
object CamelDemosBuild extends Build {
  ...
  override lazy val settings = super.settings ++
    Seq(
      ...
      resolvers += "Spring Libs Remote" at "http://repo.spring.io/libs-release-remote"
    )
  ...
  lazy val restDSL = addProject("rest-dsl") settings (libraryDependencies ++= Seq(camelJetty, camelSql)) dependsOn common
  lazy val enrich = addProject("enrich") dependsOn common
  lazy val httpComponent = addProject("http") settings (libraryDependencies += camelHttp) dependsOn common
  lazy val jbpmComponent = addProject("jbpm") settings(
    libraryDependencies ++= Seq(camelJbpm),
    libraryDependencies ++= jbpmnSeq
    ) dependsOn common
